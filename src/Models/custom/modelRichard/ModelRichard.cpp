...
ModelRichard::ModelRichard(Shader& inShader) : Model(inShader) {
...
void ModelRichard::allocateObjectData() {
	...
	for (int r = 0; r < ROWS; r++) {
		for (int c = 0; c < COLUMNS; c++) {
			...
			if (colorIndex == NONE) {
				data.cubePositions[8 - r][c][0] = colorIndex;
			}
		}
	}
	...
	for (int r = 0; r < ROWS; r++) {
		for (int c = 0; c < COLUMNS; c++) {
			...
			data.cubePositions[8 - r][c][3] = colorIndex;
		}
	}
	...
	for (int r = 0; r < ROWS; r++) {
		for (int c = 0; c < COLUMNS; c++) {
			...
			data.cubePositions[8 - r][c][4] = colorIndex;
		}
	}
	...
	for (int r = 0; r < ROWS; r++) {
		for (int c = 0; c < COLUMNS; c++) {
			...
			data.cubePositions[8 - r][c][5] = colorIndex;
		}
	}
}
...
void ModelRichard::generateRandomModel() {
	...
	for (int r = 0; r < rows; r++) {
		for (int c = 0; c < columns; c++) {
			...
			if (data.cubePositions[r][c][0] == NONE && (rand() % 100 > 5)) {
				int start = rand() % (planes - 1);
				int end = rand() % (planes - start) + start;
				for (int p = start; p < end; p++) {
					data.cubePositions[r][c][p] = (rand() % (NUM_COLORS - 1)) + 1;
				}
