...
Letter::Letter(Transform& trans) : ModelBase(trans)
{
	GenerateLetter();
}
...
void Letter::Draw(Shader& shader)
{
	...
	cubeTransform.SetPosition(transform->GetPosition() + glm::vec3(xTranslation * scaleFactor,
																   yTranslation * scaleFactor + 0.5f,
																   zTranslation * scaleFactor));
	cubeTransform.SetRotation(transform->GetRotation());
	cubeTransform.SetScale(transform->GetScale());
	...
	for (int c = 0; c < sizeX; c++)
	{
		for (int r = 0; r < sizeY; r++)
		{
				...
				model = glm::rotate(cubeTransform.GetModel(), orientation, glm::vec3(0.0f, 1.0f, 0.0f));
...
void Letter::Draw2(Shader& shader)
{
	Transform cubeTransform = Transform();
	cubeTransform.SetPosition(transform->GetPosition() + glm::vec3(xTranslation * scaleFactor,
		yTranslation * scaleFactor + 0.5f,
		zTranslation * scaleFactor));
	cubeTransform.SetRotation(transform->GetRotation());
	cubeTransform.SetScale(transform->GetScale() *1.001f);
	// Set the world position of the model
	...
	for (int c = 0; c < sizeX; c++)
	{
		for (int r = 0; r < sizeY; r++)
		{
			...
			switch (colorMode) {
			case ColorMode::col1:
				shader.setVec3("dirLight.ambient", glm::vec3(0.5f, 0.6f, 0.6f));
				break;
			case ColorMode::col2:
				shader.setVec3("dirLight.ambient", glm::vec3(0.5f, 0.4f, 0.8f));
				break;
			case ColorMode::col3:
				shader.setVec3("dirLight.ambient", glm::vec3(0.6f, 0.9f, 1.0f));
				break;
			case ColorMode::col4:
				shader.setVec3("dirLight.ambient", glm::vec3(0.2f, 0.3f, 1.0f));
				break;
			case ColorMode::col5:
				shader.setVec3("dirLight.ambient", glm::vec3(0.8f, 1.0f, 0.3f));
				break;
